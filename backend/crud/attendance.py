...
async def list_attendance_logs(
    ...
    for row in res.all():
        ...
        items.append({
            ...
            "clock_in": log.clock_in.replace(tzinfo=datetime.timezone.utc) if log.clock_in.tzinfo is None else log.clock_in,
            "clock_out": log.clock_out.replace(tzinfo=datetime.timezone.utc) if (log.clock_out and log.clock_out.tzinfo is None) else log.clock_out,
